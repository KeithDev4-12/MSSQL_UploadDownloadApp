...
type
  TUMainForm = class(TForm)
    ...
    DBGridEh8: TDBGridEh;
    vtRouteSelected: TVirtualTable;
    vtRouteSelected_id: TAutoIncField;
    vtRouteSelectedZoneCode: TWideStringField;
    vtRouteSelectedZoneName: TWideStringField;
    vtRouteSelectedReadingStartDate: TWideStringField;
    vtRouteSelectedTotalReadingDays: TWideStringField;
    vtRouteSelectedBillPeriod: TWideStringField;
    vtRouteSelectedMRNo: TIntegerField;
    PMRouteSelected: TPopupMenu;
    ...
    function isPostedToMSSQL_V2(AMR_Sys_No,AMidentity:Integer):Boolean;
...
function TUMainForm.isPostedToMSSQL_V2(AMR_Sys_No,
  AMidentity: Integer): Boolean;
Var
  MR_Sys_No_First,MR_Sys_No_Second:Integer;
  ADateTimeNew:String;
  RecordCount:Integer;
  ...
  with DMMainModule do begin
    try
      ...
      while not qryPostingMeterReading.EOF do begin
        ...
        qryMeterReadingCheck.Close;
        qryMeterReadingCheck.ParamByName('AAccountNumber').AsString := qryPostingMeterReadingAccountNo.AsString;
        qryMeterReadingCheck.ParamByName('APrevReading').AsString := qryPostingMeterReadingPreviousReading.AsString;
        qryMeterReadingCheck.ParamByName('ACurReading').AsString := qryPostingMeterReadingPresentReading.AsString;
        qryMeterReadingCheck.Open();
        qryMeterReadingCheck.First;
        //ShowMessage(qryMeterReadingCheckAcct_No.AsString  );
        ...
        //if tblSQLMeterReading.Locate('MR_Sys_no',qryMeterReadingCheckMR_Sys_no.AsString,[]) then begin
        if DaysBetween(qryPostingMeterReadingFirstReadingDate.AsDateTime,Now()) <= 1 then begin
          //if qryMeterReadingCheck.IsEmpty then begin
...
procedure TUMainForm.DeleteThisRecord1Click(Sender: TObject);
begin
  vtRouteSelected.Delete;
end;
...
begin
  ...
  vtRouteSelected.Close;
  vtRouteSelected.Open;
  ...
  with DMMainModule do begin
    ...
    scGPComboBox4.Items.Clear;
    scGPComboBox4.Items.Add;
    scGPComboBox4.Items.Items[I].Header := True;
    scGPComboBox4.Items.Items[I].Caption := 'Route Address';
    I := I + 1;
    ...
    qryMSZoneCode.Open;
    qryMSZoneCode.First;
    while not qryMSZoneCode.EOF do begin
      scGPComboBox4.Items.Add;
      scGPComboBox4.Items.Items[I].Caption := qryMSZoneCodeZoneCode.AsString + '-' + qryMSZoneCodeZoneName.AsString;
      qryMSZoneCode.Next;
      I := I + 1;
    end;
...
begin
  //if not IsMouseButtonDown(VK_LBUTTON) then begin
    ...
    if (Self.Width >= (Screen.Width-100)) AND (Self.Height >= (Screen.Height-100)) then begin
      ...
      scGPPanel14.Height := Round(Self.Height*0.45);
      ...
      scGPPanel14.Height := Round(Self.Height*0.45); //297 267;
...
begin
  ...
  if StrToInt(ADay) >= 10 then begin
    ...
    if StrToInt(AMonth) = 1 then begin
      AYear := SplitString(ADateString,'-')[0].Substring(0,4);
      ABillMonth := IntToStr(StrToInt(AYear)-1) + '12';
    end else begin
...
begin
  ...
  //GetTheMRNo From Meter Reader Accounts
    ...
    if (Length(Edit2.Text)=6) AND (Length(scGPComboEdit1.Items[scGPComboEdit1.ItemIndex].Caption)>0)  then begin
      ...
      scGPComboBox4.SetFocus;
      scGPComboBox4.DropDown;
    end;
  end;
end;
...
begin
  with DMMainModule do begin
    ...
    // Replace fdMeterReaderSchedule with vtRouteSelected
    vtRouteSelected.First;
    while not vtRouteSelected.EOF do begin
      //if I = 0 then begin
      scGPCircledProgressBar2.Value := 0;
      qryMSClients.Close;
      qryMSClients.ParamByName('AZoneCode').AsString :=  vtRouteSelectedZoneCode.AsString;
      qryMSClients.Open();
      if not qryMSClients.IsEmpty then begin
        ...
        BMClients.Execute;
        Label1.Caption := Label1.Caption + #13#10 + 'Done Clients For Zone '+ #13#10 + vtRouteSelectedZoneCode.AsString + '-' + fdMeterReaderScheduleZoneName.AsString;
      end;
      vtRouteSelected.Next;
    end;
  end;
end;
...
begin
  with DMMainModule do begin
    //replace fdMeterReaderSchedule to vtRouteSelected
    vtRouteSelected.First;
    scGPCircledProgressBar2.Value := 0;
    scGPCircledProgressBar2.Value := 5000;
    ...
    while not vtRouteSelected.EOF do begin
      ...
      qryMSMeterReadingSchedule.ParamByName('AZoneCode').AsString :=  vtRouteSelectedZoneCode.AsString; //fdMeterReaderScheduleZoneCode.AsString;
      qryMSMeterReadingSchedule.Open();
      if not qryMSMeterReadingSchedule.IsEmpty then begin
        Application.ProcessMessages;
        BMMeterReadingSchedule.Execute;
        Label1.Caption := Label1.Caption + #13#10 + 'Done Rading Schedule For Zone '+ #13#10 + vtRouteSelectedZoneCode.AsString + '-' + vtRouteSelectedZoneName.AsString;
      end;
      vtRouteSelected.Next;
    end;
  end;
...
begin
  ...
  DMMainModule.tblMeterReadingScheduleReadingStartDate.AsString := (vtRouteSelectedReadingStartDate.AsString);
end;
...
procedure TUMainForm.SpeedButton29Click(Sender: TObject);
Var
  ZoneCode,ZOneName : String;
begin
  ZoneCode := SplitString(scGPComboBox4.Items[scGPComboBox4.ItemIndex].Caption,'-')[0];
  ZoneName := SplitString(scGPComboBox4.Items[scGPComboBox4.ItemIndex].Caption,'-')[1];
  if not vtRouteSelected.Locate('ZoneCode',ZoneCode,[]) then begin
    vtRouteSelected.Insert;
    vtRouteSelectedZoneCode.AsString := ZoneCOde;
    vtRouteSelectedZoneName.AsString := ZoneName;
    vtRouteSelectedReadingStartDate.AsString := DateTimeToStr(Now());
    vtRouteSelectedTotalReadingDays.AsString := '1';
    vtRouteSelectedBillPeriod.AsString := Edit2.Text;
    vtRouteSelectedMRNo.AsString :=  '00000';

    vtRouteSelected.Post;
  end;
  ...
  with DMMainConnection do begin
    ...
    with DMMainModule do begin
      ...
      if UpperCase(scGPComboBox2.Items[scGPComboBox2.ItemIndex].Caption).Contains(UpperCase(qryDetailsMeterReadingMeterReadername.AsString)) then begin
        ...
        // save to dbfetched
        if isPostedToMSSQL_V2(MR_Sys_No,midentity) then begin
           // post dbfetched!!!
           ...
           // Not Posted!
           MessageDlg('No Available Data, Downloading Data to MSSQL Interrupted!!  ' + #13#10 +
