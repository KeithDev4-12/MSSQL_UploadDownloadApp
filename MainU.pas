...
uses
  ...
  Vcl.Menus,StrUtils,CommandPromtUnit,U_Usb, Vcl.FileCtrl;
...
begin
  ...
  if not TFile.Exists(SourceDBFile) then begin
    ...
    Memo2.Text := TCMDPromtUtil.GetDosOutput('adb push "' + SourceDBFile + '" "'+TABLET_DEFAULT_UPLOAD_LOCATION + ROOT_FOLDER_NAME +'"');
...
begin
  ...
  //GetTheMRNo From Meter Reader Accounts scGPComboBox4.Items[scGPComboBox4.ItemIndex].Caption
    ...
    if (Length(scGPComboBox3.Items[scGPComboBox3.ItemIndex].Caption)>0) AND (Length(Edit15.text)=6) then begin
      ...
      if not VTReadingScheduleMain.isEmpty then begin
...
begin
  ...
  if scSplitView2.Opened then begin
